...
function actualizarPaginacion() {
  ...
  // Si el límite inferior no llega a 1, crear botón para ir a la página 1
  if (limiteInferior > 1) {
    const botonPrimeraPagina = document.createElement('button');
    botonPrimeraPagina.textContent = 1;
    botonPrimeraPagina.classList.add('pagina');
    if (paginaActual === 1) {
      botonPrimeraPagina.classList.add('pagina-actual');
    }
    botonPrimeraPagina.addEventListener('click', () => {
      cargarPagina(1);
    });
    paginacion.insertBefore(botonPrimeraPagina, paginacion.firstChild);
  }
}
